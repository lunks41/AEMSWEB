...
namespace AMESWEB.Areas.Master.Data.Services
{
    public sealed class VesselService : IVesselService
    {
        ...
        public async Task<VesselViewModelCount> GetVesselListAsync(short CompanyId, short UserId, int pageSize, int pageNumber, string searchString)
        {
            ...
            try
            {
                var totalcount = await _repository.GetQuerySingleOrDefaultAsync<SqlResponceIds>($"SELECT COUNT(*) AS CountId FROM M_Vessel M_Vess WHERE (M_Vess.VesselName LIKE '%{searchString}%' OR M_Vess.VesselCode LIKE '%{searchString}%' OR M_Vess.VesselType LIKE '%{searchString}%') AND M_Vess.VesselId <>0 AND M_Vess.CompanyId IN (SELECT distinct CompanyId FROM Fn_Adm_GetShareCompany({CompanyId},{(short)E_Modules.Master},{(short)E_Master.Vessel}))");
        ...
        public async Task<SqlResponce> SaveVesselAsync(short CompanyId, short UserId, M_Vessel m_Vessel)
        {
            ...
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if ((codeExist?.IsExist ?? 0) > 0)
                        return new SqlResponce { Result = -1, Message = "Vessel Code already exists." };
                    ...
                    if ((nameExist?.IsExist ?? 0) > 0)
                        return new SqlResponce { Result = -1, Message = "Vessel Name already exists." };
                    ...
                    if (IsEdit)
                    {
                        ...
                        {
                            return new SqlResponce { Result = -1, Message = "Vessel Not Found" };
                        }
                    }
                    else
                    {
        ...
        public async Task<SqlResponce> DeleteVesselAsync(short CompanyId, short UserId, int vesselId)
        {

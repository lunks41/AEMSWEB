...
namespace AEMSWEB.Areas.Master.Data.Services
{
    public sealed class TaskService : ITaskService
    {
        ...
        public async Task<TaskViewModelCount> GetTaskListAsync(short CompanyId, short UserId, short pageSize, short pageNumber, string searchString)
        {
            TaskViewModelCount countViewModel = new TaskViewModelCount();
            try
            {
                var totalcount = await _repository.GetQuerySingleOrDefaultAsync<SqlResponseIds>($"SELECT COUNT(*) AS CountId FROM M_Task M_Vess WHERE (M_Vess.TaskName LIKE '%{searchString}%' OR M_Vess.TaskCode LIKE '%{searchString}%' OR M_Vess.TaskType LIKE '%{searchString}%') AND M_Vess.TaskId <>0 AND M_Vess.CompanyId IN (SELECT distinct CompanyId FROM Fn_Adm_GetShareCompany({CompanyId},{(short)E_Modules.Master},{(short)E_Master.Task}))");
        ...
        public async Task<SqlResponse> SaveTaskAsync(short CompanyId, short UserId, M_Task m_Task)
        {
            ...
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if ((codeExist?.IsExist ?? 0) > 0)
                        return new SqlResponse { Result = -1, Message = "Task Code already exists." };
                    ...
                    if ((nameExist?.IsExist ?? 0) > 0)
                        return new SqlResponse { Result = -1, Message = "Task Name already exists." };
                    ...
                    if (IsEdit)
                    {
                        ...
                        {
                            return new SqlResponse { Result = -1, Message = "Task Not Found" };
                        }
                    }
                    else
                    {
        ...
        public async Task<SqlResponse> DeleteTaskAsync(short CompanyId, short UserId, M_Task Task)
        {
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    {
                        return new SqlResponse { Result = -1, Message = "TaskId Should be zero" };
                    }
                    return new SqlResponse();
                }
            }

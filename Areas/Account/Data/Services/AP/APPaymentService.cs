...
namespace AMESWEB.Areas.Account.Data.Services.AP
{
    public sealed class APPaymentService : IAPPaymentService
    {
        ...
        public async Task<SqlResponse> SaveAPPaymentAsync(short CompanyId, ApPaymentHd arPaymentHd, List<ApPaymentDt> arPaymentDts, short UserId)
        {
            bool IsEdit = false;
            ...
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (IsEdit)
                    {
                        var dataExist = await _repository.GetQueryAsync<SqlResponseIds>($"SELECT 1 AS IsExist FROM dbo.ApPaymentHd WHERE IsCancel=0 And CompanyId={CompanyId} And PaymentId={arPaymentHd.PaymentId}");
                    ...
                    if (!IsEdit)
                    {
                        var documentIdNo = await _repository.GetQueryAsync<SqlResponseIds>($"exec S_GENERATE_NUMBER_NOANDID {CompanyId},{(short)E_Modules.AP},{(short)E_AP.Payment},'{accountDate}'");
                    ...
                    else
                    {
                        ...
                        await _repository.GetQueryAsync<SqlResponseIds>($"exec FIN_AR_CreateHistoryRec {CompanyId},{UserId},{arPaymentHd.PaymentId},{(short)E_AP.Payment}");
                    }
                    ...
                    if (SaveHeader > 0)
                    {
                        ...
                        if (SaveDetails > 0)
                        {
                            ...
                            await _repository.GetQueryAsync<SqlResponseIds>($"exec FIN_AR_CreateStatement {CompanyId},{UserId},{arPaymentHd.PaymentId},{(short)E_AP.Payment}");
        ...
        public async Task<SqlResponse> DeleteAPPaymentAsync(short CompanyId, long PaymentId, string PaymentNo, string CanacelRemarks, short UserId)
        {
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    if (PaymentId > 0)
                    {
                        ...
                        if (APPaymentToRemove > 0)
                        {
                            ...
                            await _repository.GetQueryAsync<SqlResponseIds>($"exec FIN_AR_DeleteStatement {CompanyId},{UserId},{PaymentId},{(short)E_AP.Payment}");

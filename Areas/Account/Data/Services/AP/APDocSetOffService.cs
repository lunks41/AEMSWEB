...
namespace AEMSWEB.Areas.Account.Data.Services.AP
{
    public sealed class APDocSetOffService : IAPDocSetOffService
    {
        ...
        public async Task<SqlResponse> SaveAPDocSetOffAsync(short CompanyId, ApDocSetOffHd arDocSetOffHd, List<ApDocSetOffDt> arDocSetOffDts, short UserId)
        {
            bool IsEdit = false;
        ...
        public async Task<SqlResponse> DeleteAPDocSetOffAsync(short CompanyId, long SetoffId, string SetoffNo, string CanacelRemarks, short UserId)
        {
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))

...
namespace AMESWEB.Areas.Account.Data.Services.AP
{
    public sealed class APRefundService : IAPRefundService
    {
        ...
        public async Task<SqlResponse> SaveAPRefundAsync(short CompanyId, ApRefundHd arRefundHd, List<ApRefundDt> arRefundDts, short UserId)
        {
            bool IsEdit = false;
            ...
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (IsEdit)
                    {
                        var dataExist = await _repository.GetQueryAsync<SqlResponseIds>($"SELECT 1 AS IsExist FROM dbo.ApRefundHd WHERE IsCancel=0 And CompanyId={CompanyId} And RefundId={arRefundHd.RefundId}");
                    ...
                    if (!IsEdit)
                    {
                        var documentIdNo = await _repository.GetQueryAsync<SqlResponseIds>($"exec S_GENERATE_NUMBER_NOANDID {CompanyId},{(short)E_Modules.AP},{(short)E_AR.Refund},'{accountDate}'");
        ...
        public async Task<SqlResponse> DeleteAPRefundAsync(short CompanyId, long RefundId, string RefundNo, string CanacelRemarks, short UserId)
        {
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))

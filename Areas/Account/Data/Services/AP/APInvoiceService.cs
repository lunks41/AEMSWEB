...
namespace AMESWEB.Areas.Account.Data.Services.AP
{
    public sealed class APInvoiceService : IAPInvoiceService
    {
        ...
        public async Task<SqlResponce> SaveAPInvoiceAsync(short CompanyId, ApInvoiceHd ApInvoiceHd, List<ApInvoiceDt> ApInvoiceDt, short UserId)
        {
            bool IsEdit = false;
            ...
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (IsEdit)
                    {
                        var dataExist = await _repository.GetQueryAsync<SqlResponceIds>($"SELECT 1 AS IsExist FROM dbo.ApInvoiceHd WHERE IsCancel=0 And CompanyId={CompanyId} And InvoiceId={ApInvoiceHd.InvoiceId}");
                    ...
                    if (!IsEdit)
                    {
                        var documentIdNo = await _repository.GetQueryAsync<SqlResponceIds>($"exec S_GENERATE_NUMBER_NOANDID {CompanyId},{(short)E_Modules.AP},{(short)E_AP.Invoice},'{accountDate}'");
                    ...
                    {
                        await _repository.GetQueryAsync<SqlResponceIds>($"exec FIN_AP_CreateHistoryRec {CompanyId},{UserId},{ApInvoiceHd.InvoiceId},{(short)E_AP.Invoice}");
                    }
                    ...
                    if (SaveHeader > 0)
                    {
                        ...
                        if (SaveDetails > 0)
                        {
                            ...
                            await _repository.GetQueryAsync<SqlResponceIds>($"exec FIN_AP_CreateStatement {CompanyId},{UserId},{ApInvoiceHd.InvoiceId},{(short)E_AP.Invoice}");
        ...
        public async Task<SqlResponce> DeleteAPInvoiceAsync(short CompanyId, long InvoiceId, string CanacelRemarks, short UserId)
        {
                ...
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (InvoiceId > 0)
                    {
                        ...
                        if (APInvoiceToRemove > 0)
                        {
                            ...
                            await _repository.GetQueryAsync<SqlResponceIds>($"exec FIN_AP_DeleteStatement {CompanyId},{UserId},{InvoiceId},{(short)E_AP.Invoice}");

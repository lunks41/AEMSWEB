...
namespace AMESWEB.Areas.Account.Data.Services.AR
{
    public sealed class ARCreditNoteService : IARCreditNoteService
    {
        ...
        public async Task<SqlResponce> SaveARCreditNoteAsync(short CompanyId, ArCreditNoteHd arCreditNoteHd, List<ArCreditNoteDt> arCreditNoteDt, short UserId)
        {
            bool IsEdit = false;
            ...
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (IsEdit)
                    {
                        var dataExist = await _repository.GetQueryAsync<SqlResponceIds>($"SELECT 1 AS IsExist FROM dbo.ArCreditNoteHd WHERE IsCancel=0 And CompanyId={CompanyId} And CreditNoteId={arCreditNoteHd.CreditNoteId}");
                    ...
                    if (!IsEdit)
                    {
                        var documentIdNo = await _repository.GetQueryAsync<SqlResponceIds>($"exec S_GENERATE_NUMBER_NOANDID {CompanyId},{(short)E_Modules.AR},{(short)E_AR.CreditNote},'{accountDate}'");
                    ...
                    {
                        await _repository.GetQueryAsync<SqlResponceIds>($"exec FIN_AR_CreateHistoryRec {CompanyId},{UserId},{arCreditNoteHd.CreditNoteId},{(short)E_AR.CreditNote}");
                    }
                    ...
                    if (SaveHeader > 0)
                    {
                        ...
                        if (SaveDetails > 0)
                        {
                            ...
                            await _repository.GetQueryAsync<SqlResponceIds>($"exec FIN_AR_CreateStatement {CompanyId},{UserId},{arCreditNoteHd.CreditNoteId},{(short)E_AR.CreditNote}");
        ...
        public async Task<SqlResponce> DeleteARCreditNoteAsync(short CompanyId, long CreditNoteId, string CanacelRemarks, short UserId)
        {
                ...
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (CreditNoteId > 0)
                    {
                        ...
                        if (ARCreditNoteToRemove > 0)
                        {
                            ...
                            await _repository.GetQueryAsync<SqlResponceIds>($"exec FIN_AR_DeleteStatement {CompanyId},{UserId},{CreditNoteId},{(short)E_AR.CreditNote}");

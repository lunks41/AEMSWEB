    ...
    public sealed class ARRefundService : IARRefundService
    {
        ...
        public async Task<ARRefundViewModelCount> GetARRefundListAsync(short CompanyId, int pageSize, int pageNumber, string searchString, string fromDate, string toDate, short UserId)
        {
            ...
            try
            {
                ...
                countViewModel.data = result?.ToList() ?? new List<ARRefundViewModel>();
        ...
        public async Task<SqlResponce> SaveARRefundAsync(short CompanyId, ArRefundHd arRefundHd, List<ArRefundDt> arRefundDts, short UserId)
        {
            bool IsEdit = false;
            ...
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (IsEdit)
                    {
                        var dataExist = await _repository.GetQueryAsync<SqlResponceIds>($"SELECT 1 AS IsExist FROM dbo.ArRefundHd WHERE IsCancel=0 And CompanyId={CompanyId} And RefundId={arRefundHd.RefundId}");
                    ...
                    if (!IsEdit)
                    {
                        var documentIdNo = await _repository.GetQueryAsync<SqlResponceIds>($"exec S_GENERATE_NUMBER_NOANDID {CompanyId},{(short)E_Modules.AR},{(short)E_AR.Refund},'{accountDate}'");
        ...
        public async Task<SqlResponce> DeleteARRefundAsync(short CompanyId, long RefundId, string RefundNo, string CanacelRemarks, short UserId)
        {
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))

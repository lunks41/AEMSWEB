...
namespace AEMSWEB.Services.Accounts.GL
{
    public sealed class GLOpeningBalanceService : IGLOpeningBalanceService
    {
        ...
        public async Task<SqlResponse> SaveGLOpeningBalanceAsync(Int16 CompanyId, GLOpeningBalanceViewModel gLOpeningBalanceViewModel, Int16 UserId)
        {
        ...
        public async Task<IEnumerable<GLOpeningBalanceViewModel>> GenerateGLYearEndProcessAsyn(Int16 CompanyId, Int32 DocumentId, Int16 UserId)
        {
            try
            {
                return await _repository.GetQueryAsync<GLOpeningBalanceViewModel>($"exec Adm_GetUserTransactions {CompanyId},{DocumentId},{UserId}");
            }
            catch (Exception ex)
            {
                var errorLog = new AdmErrorLog

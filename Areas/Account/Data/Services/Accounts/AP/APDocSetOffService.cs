...
namespace AEMSWEB.Services.Accounts.AP
{
    public sealed class APDocSetOffService : IAPDocSetOffService
    {
        ...
        public async Task<APDocSetOffViewModel> GetAPDocSetOffByIdAsync(Int16 CompanyId, Int64 SetoffId, string SetoffNo, Int16 UserId)
        {
        ...
        public async Task<SqlResponse> SaveAPDocSetOffAsync(Int16 CompanyId, ApDocSetOffHd arDocSetOffHd, List<ApDocSetOffDt> arDocSetOffDts, Int16 UserId)
        {
            bool IsEdit = false;
            ...
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (IsEdit)
                    {
                        var dataExist = await _repository.GetQueryAsync<SqlResponseIds>($"SELECT 1 AS IsExist FROM dbo.ApDocSetOffHd WHERE IsCancel=0 And CompanyId={CompanyId} And SetoffId={arDocSetOffHd.SetoffId}");
                    ...
                    if (!IsEdit)
                    {
                        var documentIdNo = await _repository.GetQueryAsync<SqlResponseIds>($"exec S_GENERATE_NUMBER_NOANDID {CompanyId},{(short)E_Modules.AP},{(short)E_AR.DocSetoff},'{accountDate}'");
        ...
        public async Task<SqlResponse> DeleteAPDocSetOffAsync(Int16 CompanyId, Int64 SetoffId, string SetoffNo, string CanacelRemarks, Int16 UserId)
        {
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
        ...
        public async Task<IEnumerable<APDocSetOffViewModel>> GetHistoryAPDocSetOffByIdAsync(Int16 CompanyId, Int64 SetoffId, string SetoffNo, Int16 UserId)
        {
            try
            {

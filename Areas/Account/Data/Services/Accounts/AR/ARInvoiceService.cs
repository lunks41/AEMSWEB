...
namespace AEMSWEB.Services.Accounts.AR
{
    public sealed class ARInvoiceService : IARInvoiceService
    {
        ...
        public ARInvoiceService(IRepository<ArInvoiceHd> repository, ApplicationDbContext context, ILogService logService, IAccountService accountService)
        {
        ...
        public async Task<SqlResponse> SaveARInvoiceAsync(Int16 CompanyId, Int16 UserId, ArInvoiceHd arInvoiceHd, List<ArInvoiceDt> arInvoiceDt)
        {
            bool IsEdit = false;
            ...
            try
            {
                using (var TScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (IsEdit)
                    {
                        var dataExist = await _repository.GetQueryAsync<SqlResponseIds>($"SELECT 1 AS IsExist FROM dbo.ArInvoiceHd WHERE IsCancel=0 And CompanyId={CompanyId} And InvoiceId={arInvoiceHd.InvoiceId}");
                    ...
                    if (!IsEdit)
                    {
                        var documentIdNo = await _repository.GetQueryAsync<SqlResponseIds>($"exec S_GENERATE_NUMBER_NOANDID {CompanyId},{(short)E_Modules.AR},{(short)E_AR.Invoice},'{accountDate}'");
                    ...
                    else
                    {
                        ...
                        await _repository.GetQueryAsync<SqlResponseIds>($"exec FIN_AR_CreateHistoryRec {CompanyId},{UserId},{arInvoiceHd.InvoiceId},{(short)E_AR.Invoice}");

                        //arInvoiceHd.EditVersion = Convert.ToByte(arInvoiceHd.EditVersion + 1);
                    }
                    ////Document Id Generation
                    //Int64 InvoiceId = await _accountService.GenrateDocumentId( (short)E_Modules.AR, (short)E_AR.Invoice);

                    ////Document Number Generation
                    //string InvoiceNo = await _accountService.GenrateDocumentNumber( CompanyId, (short)E_Modules.AR, (short)E_AR.Invoice, arInvoiceHd.AccountDate);
                    ...
                    if (SaveHeader > 0)
                    {
                        ...
                        if (SaveDetails > 0)
                        {
                            ...
                            await _repository.GetQueryAsync<SqlResponseIds>($"exec FIN_AR_CreateStatement {CompanyId},{UserId},{arInvoiceHd.InvoiceId},{(short)E_AR.Invoice}");
                            ...
                            if (auditLogSave > 0)
                            {
                                //Update Edit Version
                                if (IsEdit)
                                    await _repository.UpsertExecuteScalarAsync($"update ArinvoiceHd set EditVersion=EditVersion+1 where InvoiceId={arInvoiceHd.InvoiceId}; Update ArInvoiceDt set EditVersion=(SELECT TOP 1 EditVersion FROM dbo.ArInvoiceHd where InvoiceId={arInvoiceHd.InvoiceId}) where InvoiceId={arInvoiceHd.InvoiceId}");

                                //Create / Update Ar Statement
                                await _repository.GetQueryAsync<SqlResponseIds>($"exec FIN_AR_CreateStatement {CompanyId},{UserId},{arInvoiceHd.InvoiceId},{(short)E_AR.Invoice}");

                                TScope.Complete();
                                return new SqlResponse { Result = arInvoiceHd.InvoiceId, Message = "Save Successfully" };
                            }
                        }
                        else
                        {
        ...
        public async Task<SqlResponse> DeleteARInvoiceAsync(Int16 CompanyId, Int16 UserId, Int64 InvoiceId, string CanacelRemarks)
        {
            ...
            try
            {
                ...
                {
                    //Get Invoice Number
                    InvoiceNo = await _repository.GetQuerySingleOrDefaultAsync<string>($"SELECT InvoiceNo FROM dbo.ArInvoiceHd WHERE InvoiceId={InvoiceId}");
                    ...
                    if (InvoiceId > 0)
                    {
                        ...
                        if (ARInvoiceToRemove > 0)
                        {
                            ...
                            await _repository.GetQueryAsync<SqlResponseIds>($"exec FIN_AR_DeleteStatement {CompanyId},{UserId},{InvoiceId},{(short)E_AR.Invoice}");
        ...
        public async Task<IEnumerable<ARInvoiceViewModel>> GetHistoryARInvoiceByIdAsync(Int16 CompanyId, Int16 UserId, Int64 InvoiceId, string InvoiceNo)
        {
            try
            {
                return await _repository.GetQueryAsync<ARInvoiceViewModel>($"SELECT Invhd.EditVersion, Invhd.CompanyId,Invhd.InvoiceId,Invhd.InvoiceNo,Invhd.ReferenceNo,Invhd.TrnDate,Invhd.AccountDate,Invhd.DeliveryDate,Invhd.DueDate,Invhd.CustomerId,M_Cus.CustomerCode,M_Cus.CustomerName,Invhd.CurrencyId,M_Cur.CurrencyCode,M_Cur.CurrencyCode,Invhd.ExhRate,Invhd.CtyExhRate,Invhd.CreditTermId,M_Crd.CreditTermCode,M_Crd.CreditTermName,Invhd.BankId,M_Ban.BankCode,M_Ban.BankName,Invhd.TotAmt,Invhd.TotLocalAmt,Invhd.TotCtyAmt,Invhd.GstClaimDate,Invhd.GstAmt,Invhd.GstLocalAmt,Invhd.GstCtyAmt,Invhd.TotAmtAftGst,Invhd.TotLocalAmtAftGst,Invhd.TotCtyAmtAftGst,Invhd.BalAmt,Invhd.BalLocalAmt,Invhd.PayAmt,Invhd.PayLocalAmt,Invhd.ExGainLoss,Invhd.SalesOrderId,Invhd.SalesOrderNo,Invhd.OperationId,Invhd.OperationNo,Invhd.Remarks,Invhd.Address1,Invhd.Address2,Invhd.Address3,Invhd.Address4,Invhd.PinCode,Invhd.CountryId,M_Cun.CountryCode,M_Cun.CountryName,Invhd.PhoneNo,Invhd.FaxNo,Invhd.ContactName,Invhd.MobileNo,Invhd.EmailAdd,Invhd.ModuleFrom,Invhd.SupplierName,Invhd.SuppInvoiceNo,Invhd.APInvoiceId,Invhd.APInvoiceNo,Invhd.CreateById,Invhd.CreateDate,Invhd.EditById,Invhd.EditDate,Invhd.IsCancel,Invhd.CancelById,Invhd.CancelDate,Invhd.CancelRemarks,Usr.UserName AS CreateBy,Usr1.UserName AS EditBy,Usr2.UserName AS CancelBy FROM dbo.ArInvoiceHd_Ver Invhd INNER JOIN dbo.M_Customer M_Cus ON M_Cus.CustomerId = Invhd.CustomerId INNER JOIN dbo.M_Currency M_Cur ON M_Cur.CurrencyId = Invhd.CurrencyId INNER JOIN dbo.M_CreditTerm M_Crd ON M_Crd.CreditTermId = Invhd.CreditTermId INNER JOIN dbo.M_Bank M_Ban ON M_Ban.BankId = Invhd.BankId LEFT JOIN dbo.M_Country M_Cun ON M_Cun.CountryId = Invhd.CountryId LEFT JOIN dbo.AdmUser Usr ON Usr.UserId = Invhd.CreateById LEFT JOIN dbo.AdmUser Usr1 ON Usr1.UserId = Invhd.EditById LEFT JOIN dbo.AdmUser Usr2 ON Usr2.UserId = Invhd.CancelById WHERE (Invhd.InvoiceId={InvoiceId})");
            }
            catch (Exception ex)
            {
                var errorLog = new AdmErrorLog
        ...
        public async Task<ARInvoiceViewModel> GetHistoryARInvoiceByIdAsync_V1(Int16 CompanyId, Int16 UserId, Int64 InvoiceId, string InvoiceNo)
        {
            ARInvoiceViewModel aRInvoiceViewModel = new ARInvoiceViewModel();
            try
            {
                ...
                var result = await _repository.GetQueryAsync<ARInvoiceDtViewModel>($"SELECT Invdt.InvoiceId,Invdt.InvoiceNo,Invdt.ItemNo,Invdt.SeqNo,Invdt.DocItemNo,Invdt.ProductId,M_Pro.ProductCode,M_Pro.ProductName,Invdt.GLId,M_chra.GLCode,M_chra.GLName,Invdt.QTY,Invdt.BillQTY,Invdt.UomId,M_um.UomCode,M_um.UomName,Invdt.UnitPrice,Invdt.TotAmt,Invdt.TotLocalAmt,Invdt.TotCtyAmt,Invdt.Remarks,Invdt.GstId,M_Gs.GstCode,M_Gs.GstName,Invdt.GstPercentage,Invdt.GstAmt,Invdt.GstLocalAmt,Invdt.GstCtyAmt,Invdt.DeliveryDate,Invdt.DepartmentId,M_Dep.DepartmentCode,M_Dep.DepartmentName,Invdt.EmployeeId,M_Emp.EmployeeCode,M_Emp.EmployeeName,Invdt.PortId,M_Po.PortCode,M_Po.PortName,Invdt.VesselId,M_Vel.VesselCode,M_Vel.VesselName,Invdt.BargeId,M_Brg.BargeCode,M_Brg.BargeName,Invdt.VoyageId,M_Vo.VoyageNo,M_Vo.ReferenceNo as VoyageReferenceNo,Invdt.OperationId,Invdt.OperationNo,Invdt.OPRefNo,Invdt.SalesOrderId,Invdt.SalesOrderNo,Invdt.SupplyDate,Invdt.SupplierName,Invdt.SuppInvoiceNo,Invdt.APInvoiceId,Invdt.APInvoiceNo,Invdt.EditVersion FROM dbo.ArInvoiceDt_Ver Invdt LEFT JOIN dbo.M_Uom M_um ON M_um.UomId = Invdt.UomId LEFT JOIN dbo.M_ChartOfAccount M_chra ON M_chra.GLId = Invdt.GLId LEFT JOIN dbo.M_Product M_Pro ON M_Pro.ProductId = Invdt.ProductId LEFT JOIN dbo.M_Gst M_Gs ON M_Gs.GstId = Invdt.GstId LEFT JOIN dbo.M_Department M_Dep ON M_Dep.DepartmentId = Invdt.DepartmentId LEFT JOIN dbo.M_Employee M_Emp ON M_Emp.EmployeeId = Invdt.EmployeeId LEFT JOIN dbo.M_Port M_Po ON M_Po.PortId = Invdt.PortId LEFT JOIN dbo.M_Vessel M_Vel ON M_Vel.VesselId = Invdt.VesselId LEFT JOIN dbo.M_Barge M_Brg ON M_Brg.BargeId = Invdt.BargeId LEFT JOIN dbo.M_Voyage M_Vo ON M_Vo.VoyageId = Invdt.VoyageId  WHERE Invdt.InvoiceId={aRInvoiceViewModel.InvoiceId}");
